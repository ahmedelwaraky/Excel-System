// ExcelTable.jsx — مكوّن العرض فقط (UI only)
...
function AddColModal({ onConfirm, onClose }) {
  ...
  return (
    ...
    >
      <div className="bg-white rounded-xl shadow-xl w-72 p-5 flex flex-col gap-4" dir="rtl">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex-1 py-2 text-xs font-semibold text-white bg-[#1d6f42] hover:bg-[#155233] rounded-lg transition-all"
          >
            إضافة
          </button>
          <button
            onClick={onClose}
            className="flex-1 py-2 text-xs font-semibold text-gray-600 bg-gray-100 hover:bg-gray-200 rounded-lg transition-all"
          >
            إلغاء
          </button>
        </div>
 ...
 * Props:
 *  headers      string[]
 *  rows         object[]   — already filtered
 *  totalRows    number     — عدد الصفوف قبل الفلترة
 *  loading      boolean
 *  saved        "" | "saving" | "saved"
 *  uploading    boolean
 *  hasFilter    boolean
 *  currentUser  object
 *  onAddRow     (formData) => void
 *  onAddColumn  (name)     => void
 *  onDeleteRow  (id)       => void
 *  onUpload     (e)        => void
 *  onDownload   ()         => void
 */
    ...
    <div className="h-full flex flex-col bg-white" dir="rtl">
      ...
      <div className="flex items-center gap-2 px-3 py-2 bg-[#f8faf8] border-b border-[#e0e0e0] flex-shrink-0 flex-wrap">
        ...
        <button
          ...
          className="flex items-center gap-1 px-3 py-1.5 text-xs text-gray-600 bg-white hover:bg-gray-50 border border-gray-200 rounded-lg transition-all"
        >
          + عمود
        </button>
        ...
        <div className="flex items-center border border-[#c6d9c6] rounded-lg overflow-hidden">
          <button
            ...
            disabled={uploading}
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs text-[#1d6f42] bg-white hover:bg-[#f0faf0] transition-all disabled:opacity-50 border-l border-[#c6d9c6]"
          >
        ...
        <span className="mr-auto text-xs text-gray-400 flex items-center gap-2">
          {hasFilter ? `${rows.length} من ${totalRows} صف` : `${rows.length} صف`}
          {saved === "saving" && <span className="text-amber-500">● حفظ...</span>}
          {saved === "saved"  && <span className="text-[#1d6f42]">✓ تم الحفظ</span>}
        </span>
      </div>
      ...
      <div className="flex-1 overflow-auto">
            ...
            {rows.length === 0 ? (
              ...
              rows.map((row, i) => {
                ...
                return (
                  ...
                  >
                    <td className="text-center text-[11px] text-gray-300 select-none border-l border-[#f0f0f0] py-0 h-10 w-10">
                      {i + 1}
                    </td>
                    <td className="border-l border-[#f0f0f0] px-3 py-0 h-10">
                      <Cell colKey="نوع المكاتبة"   value={d["نوع المكاتبة"]} />
                    </td>
                    <td className="border-l border-[#f0f0f0] px-3 py-0 h-10">
                      <Cell colKey="رقم المكاتبة"   value={d["رقم المكاتبة"]} />
                    </td>
                    <td className="border-l border-[#f0f0f0] px-3 py-0 h-10">
                      <Cell colKey="تاريخ المكاتبة" value={d["تاريخ المكاتبة"]} />
                    </td>
                    <td className="border-l border-[#f0f0f0] px-3 py-0 h-10 max-w-0 overflow-hidden">
                      <Cell colKey="الموضوع"         value={d["الموضوع"]} />
                    </td>
                    <td className="border-l border-[#f0f0f0] px-3 py-0 h-10">
                      <Cell colKey="الجهة الرئيسية" value={d["الجهة الرئيسية"]} />
                    </td>
                    <td className="border-l border-[#f0f0f0] px-3 py-0 h-10">
                      <Cell colKey="الأولوية"        value={d["الأولوية"]} />
                    </td>
                    <td className="border-l border-[#f0f0f0] px-3 py-0 h-10">
                      <Cell colKey="الحالة"          value={d["الحالة"]} />
                    </td>
        ...
        {rows.length > 0 && (
          <button
            onClick={() => setShowForm(true)}
            className="w-full py-2.5 text-right pr-12 text-xs text-gray-300 hover:text-[#1d6f42] hover:bg-[#f6fdf7] border-t border-[#f0f0f0] transition-all"
          >
            + إضافة مكاتبة جديدة
          </button>
        )}
